...
export InputPeripheral, OutputPeripheral, Action, Input
export BOOT, INTELLIGENCE_RUNNING, ENERGY, HISTORY
...
function state(mm::ModuleMethod)
    sig = mm.method.sig
    sig isa UnionAll && (sig = Base.unwrap_unionall(sig))
    params = sig.parameters[2:end]
    ret_types = Base.return_types(mm.f, Tuple{params...})
    sig_str = split(state(mm.method), " @")[1]
    sig_str = replace(sig_str, "__source__::LineNumberNode, __module__::Module, " => "")
    binding = Docs.Binding(mm.m, nameof(mm.f))
    doc_str = haskey(Docs.meta(mm.m), binding) ? strip(string(Docs.doc(mm.f, sig))) * "\n" : ""
    doc_str * sig_str * "::$(Union{ret_types...})"
end
function state(_state::Vector{TrackedSymbol}, T::Type)
    typenames = T == ModuleMethod ? "Function:" : string(T)
    lines = String[typenames]
    for s in _state
        typeof(s.value) ≠ T && continue
        pre = T ∈ [DataType, ModuleMethod] ? "" : state(s.sym) * "="
    ...
    for sym in sort(Base.invokelatest(names, m, all=(m==Main)))
        ...
        if value isa Function
            for method in methods(value)
                method.module ≠ m && continue
                push!(_state, tracked_symbol(ModuleMethod(m, value, method)))
            end
            continue
        end
